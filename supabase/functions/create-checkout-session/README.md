...
**Secrets:** In Supabase Dashboard → Edge Functions → create-checkout-session → Secrets (or `supabase secrets set`), set: `STRIPE_SECRET_KEY`, `SITE_URL`, `STRIPE_PRICE_OPEN_MIC`, `STRIPE_PRICE_ARTIST`, `STRIPE_PRICE_LABEL`.
...
- Subscriptions (`artist`, `label`): `invoice.paid` and `customer.subscription.*` update `subscriptions`. Granting credits on subscription invoices (80 / 330) is not yet implemented; add in `stripe-webhook` if needed.
